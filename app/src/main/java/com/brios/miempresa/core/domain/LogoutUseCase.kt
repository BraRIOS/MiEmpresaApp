...
class LogoutUseCase
    ...
    suspend operator fun invoke(activity: Activity) {
        ...
        withContext(Dispatchers.IO) {
            val visitedCompanies =
                companyDao.getVisitedCompaniesList().map { company ->
                    company.copy(selected = false)
                }
            syncManager.cancelAll()
            database.clearAllTables()
            if (visitedCompanies.isNotEmpty()) {
                companyDao.upsertCompanies(visitedCompanies)
            }
        }
    }
}
